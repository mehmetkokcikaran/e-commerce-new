...
function Checkout(props) {
  ...
  async function handleSubmit(e){
    showAlert({msg:"submitting order... plese wait"});
    e.preventDefault()
    ...
    if(token){
      setError('');
      const {id} = token;
      let order = await submitOrder({name:name, total:total,
      items:cart, stripeTokenId:id,userToken:user.token})
  ...
  order total : <span>${total}</span>
        ...
        <div className="stripe-input">
          ...
          <p className="stripe-info">Test using this credit card : 
...
const StripeWrapper = () => {
  return (
    <StripeProvider apiKey="pk_test_1az04yetP1k1o1rSRT3RquEG00dDBVv9oy">
      <Elements>
        <CardForm></CardForm>
      </Elements>
    </StripeProvider>
